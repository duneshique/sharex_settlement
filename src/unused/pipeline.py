...
from .parsers.union_pdf import (
    parse_union_settlement_pdf,
    scan_union_settlement_pdfs,
)
...
# 제거됨: EXPECTED_Q4_2024 (이제 Ground Truth PDF에서 동적으로 로드함)
...
def run_quarterly_pdf_pipeline(
    ...
    # 분기 PDF 파일 찾기 (archive 디렉토리 사용)
    fc_dir = Path(base_path) / "archive" / "FastCampus_Settlement"
...
def run_monthly_pipeline(
    ...
    results = engine.calculate_settlement(
        ...
        # revenue_share_ratio와 union_payout_ratio는 이제 company 모델값을 기본으로 사용함
    )

    return results


def load_ground_truth_from_union_pdfs(
    base_path: str,
    period: str,
) -> Dict[str, float]:
    """
    Union_Profit Share_Settlement 디렉토리에서 확정 정산서 데이터를 로드하여 Ground Truth 생성
    """
    print(f"\n[Ground Truth 로드] {period}")
    gt_dir = Path(base_path) / "archive" / "Union_Profit Share_Settlement"
    
    # 분기 필터 (예: "2024-Q4" -> "4Q")
    q_filter = period.split("-")[-1].replace("Q", "") + "Q"
    
    results = scan_union_settlement_pdfs(str(gt_dir), q_filter)
    
    ground_truth = {}
    total = 0.0
    
    # 헬퍼: PDF 기업명과 JSON ID 매핑
    mapping = {
        "BKID": "bkid", "HEAZ": "heaz", "김형준": "blsn", "김형준 변리사": "blsn",
        "디자인하우스": "designhouse", "디파이": "dfy", "바른폰트": "blsn", "산돌": "sandoll",
        "시싸이드시티": "csidecity", "아뜰리에동가": "atelier_dongga", "코스믹레이": "cosmicray",
        "컴파운드컬렉티브": "compound_c", "폰트릭스": "fontrix", "허스키폭스": "huskyfox"
    }
    
    for name, data in results.items():
        cid = mapping.get(name, name.lower())
        amount = data.get("settlement_amount", 0.0)
        ground_truth[cid] = amount
        total += amount
        print(f"  {name:<15} -> {cid:<15}: {amount:>15,.1f}")
...
def validate_results(
    ...
    if not expected:
        print("[Warning] 검증할 예상 데이터(Ground Truth)가 없습니다.")
        return False

    print(f"{'기업ID':<30} {'예상':>15} {'실제':>15} {'차이':>12} {'결과'}")
    print("-" * 80)
    ...
    for cid, exp_val in sorted(expected.items()):
        # CompanySettlement 객체 또는 numeric value 처리
        val_or_obj = results.get(cid, 0.0)
        if hasattr(val_or_obj, "union_payout"):
            actual_val = val_or_obj.union_payout
        else:
            actual_val = float(val_or_obj)
        
        diff = abs(exp_val - actual_val)
        passed = diff < 10.0  # 허용 오차 10원
        ...
        print(f"{cid:<30} {exp_val:>15,.1f} {actual_val:>15,.1f} {diff:>12,.1f}   {res_str}")

    print("-" * 80)
    diff_total = abs(total_expected - total_actual)
    res_total = "PASS" if diff_total < 100.0 else "FAIL"
    print(f"{'합계':<30} {total_expected:>15,.1f} {total_actual:>15,.1f} {diff_total:>12,.1f}   {res_total}")

    if all_passed:
        print("\nALL PASSED! 정산 결과가 확정 데이터와 일치합니다.")
    else:
        print("\nSOME FAILED")
...
def _find_monthly_pdfs(fc_dir: Path, months: List[str], period: str = "") -> Dict[str, Path]:
    """월별 정산서 PDF 파일 찾기 (하위 디렉토리 포함. 분기 정산서도 포함 지원)"""
    result = {}
    q_filter = period.split("-")[-1] if period else "" # "Q4"
    
    for pdf in fc_dir.rglob("*.pdf"):
        name = unicodedata.normalize("NFC", pdf.name)
        period_type, value = detect_period_from_filename(name)
        
        if period_type == "monthly" and value in months:
            result[value] = pdf
        elif period_type == "quarterly" and q_filter in value:
            # 분기 PDF를 찾으면, 해당 분기의 모든 월에 대해 이 파일을 사용하도록 함 (임시)
            # 또는 그냥 리스트에 추가하여 파서가 처리하게 함
            for m in months:
                if m not in result:
                    result[m] = pdf
    return result
...
def _load_from_monthly_pdfs(
    ...
    """월별 FastCampus PDF에서 데이터 로드 (archive 디렉토리 사용)"""
    fc_dir = Path(base_path) / "archive" / "FastCampus_Settlement"
    monthly_pdfs = _find_monthly_pdfs(fc_dir, months, engine.period)
...
def _load_from_excel(
    ...
    master_path = Path(base_path) / "archive" / "Share X Settlement_Master_2023-2026.xlsx"
    info_path = Path(base_path) / "archive" / "Share X Settlement_Info.xlsx"
