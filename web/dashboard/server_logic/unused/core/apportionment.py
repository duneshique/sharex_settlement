...
# 데이터 모델 import
from ..models.company import Company, CompanySettlement
from ..models.course import Course, CourseSales
from ..models.campaign import CampaignCost
...
class ApportionmentEngine:
    ...
    def calculate_settlement(
        ...
        for cid in union_companies:
            ...
            # 정산 비율 로드 (기본값 설정)
            rs_ratio = company.revenue_share_ratio or 0.75
            union_payout_ratio = company.union_payout_ratio or 0.50

            # 정산 금액 계산
            if contribution > 0:
                # 수익쉐어 강사료 (전체 RS 대상 금액)
                rs_fee = contribution * rs_ratio
                # 유니온 실지급
                union_pay = contribution * union_payout_ratio
                # 플러스엑스 보유분 (RS 비대상 + RS 대상 중 잔여분)
                plusx_share = contribution - union_pay
                
                status = "normal"
                deferred_to_next_quarter = False
                deferred_amount = 0.0
            else:
                # 공헌이익 < 0 인 경우: 정산 이월
                rs_fee = 0.0
                union_pay = 0.0
                plusx_share = contribution  # 마이너스 성과 플러스엑스가 일단 떠안음
                
                status = "deferred"
                deferred_to_next_quarter = True
                deferred_amount = contribution
            ...
            settlement = CompanySettlement(
                ...
                union_payout=round(union_pay, 2),
                plusx_share=round(plusx_share, 2),
                course_count=course_count,
                indirect_ad_per_course=round(indirect_per_course, 2),
                revenue_share_ratio=rs_ratio,
                union_payout_ratio=union_payout_ratio,
                status=status,
                deferred_to_next_quarter=deferred_to_next_quarter,
